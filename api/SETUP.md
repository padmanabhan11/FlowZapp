...
| `Http/Controllers/Ai/` | FR-8xx: `POST /ai/rewrite` (proposal `{original, proposed, changed}` — never mutates; client PATCHes what the person accepts), `POST /ai/translate` (creates a linked draft; source re-approval marks it stale), `POST /ai/suggest-title`. Prompts in `app/Ai/Prompts.php` with `[rewrite]`/`[translate]`/`[title]` markers for `FakeLlm` |
| `Http/Controllers/Documents/` (Epic B completion) | `TemplateController` (built-in + workspace templates, save-from-document, delete), `DocumentAssetController` (presigned PUT for editor images/files, complete), `DocumentLinkController` (`POST /document-links/resolve`, `GET /documents/{id}/backlinks`); `document_links` is rebuilt by `DocumentObserver::syncLinks` on every content save |
