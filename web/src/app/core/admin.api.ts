...
export interface SpaceMemberRow {
  ...
  email: string | null;
  role: Role;
  source: 'space' | 'workspace';
}
export interface ChainEntry {
  level: 'workspace' | 'space' | 'folder' | 'result';
  id: string | null;
  name: string | null;
  role: string | null;
  note?: string | null;
...
export interface Resolution {
  ...
  folder_overrides?: { folder: { id: string; name: string } | null; role: string }[];
}
...
export class AdminApi {
  ...
  async members(wsId: string): Promise<Member[]> {
    return (
      await firstValueFrom(this.http.get<{ data: Member[] }>(`/api/v1/workspaces/${wsId}/members`))
    ).data;
  }
  ...
  async invites(wsId: string): Promise<PendingInvite[]> {
    return (
      await firstValueFrom(
        this.http.get<{ data: PendingInvite[] }>(`/api/v1/workspaces/${wsId}/invites`),
      )
    ).data;
  }
  ...
  async resendInvite(wsId: string, inviteId: string): Promise<void> {
    await this.session.ensureCsrf();
    await firstValueFrom(
      this.http.post(`/api/v1/workspaces/${wsId}/invites/${inviteId}/resend`, {}),
    );
  }
  ...
  async spaceMembers(spaceId: string): Promise<SpaceMemberRow[]> {
    return (
      await firstValueFrom(
        this.http.get<{ data: SpaceMemberRow[] }>(`/api/v1/spaces/${spaceId}/members`),
      )
    ).data;
  }
  ...
  async grantSpace(spaceId: string, userId: string, role: Role): Promise<void> {
    await this.session.ensureCsrf();
    await firstValueFrom(
      this.http.post(`/api/v1/spaces/${spaceId}/members`, { user_id: userId, role }),
    );
  }
  ...
  async spaceAccess(spaceId: string, userId: string): Promise<Resolution> {
    return (
      await firstValueFrom(
        this.http.get<{ data: Resolution }>(`/api/v1/spaces/${spaceId}/access`, {
  ...
  async folderAcl(folderId: string): Promise<FolderAcl> {
    return (
      await firstValueFrom(
        this.http.get<{ data: FolderAcl }>(`/api/v1/folders/${folderId}/permissions`),
      )
    ).data;
  }
  ...
  async folderChain(folderId: string, userId: string): Promise<Resolution> {
    return (
      await firstValueFrom(
        this.http.get<{ data: Resolution }>(`/api/v1/folders/${folderId}/permissions`, {
          params: { user_id: userId },
        }),
      )
    ).data;
  ...
  async setFolderOverride(folderId: string, userId: string, role: string): Promise<void> {
    await this.session.ensureCsrf();
    await firstValueFrom(
      this.http.put(`/api/v1/folders/${folderId}/permissions/${userId}`, { role }),
    );
  }
