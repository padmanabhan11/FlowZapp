...
describe('uploadParts', () => {
  it('PUTs each part to its presigned URL, retries a failed part, and returns ETags in order', async () => {
    ...
    const targets = { recording_id: 'r', upload_id: 'u', part_size: 4, parts: [1, 2, 3].map((n) => ({ part_number: n, url: `https://s/${n}` })) };
    let calls = 0;
    ...
    expect(out).toEqual([{ part_number: 1, etag: 'etag-1' }, { part_number: 2, etag: 'etag-2' }, { part_number: 3, etag: 'etag-3' }]);
    expect(calls).toBe(4);
